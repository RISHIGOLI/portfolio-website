...
function Chat({ chat, selected }) {
    ...
    return (
        ...
        <Button className={classes.mainContainer} style={{ color: selected && 'white', fontWeight: selected && '600' }}>
            {/* badge container */}
            <Grid style={{ height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <Box style={{ height: '3.5rem', width: '3.5rem', borderRadius: '50%', backgroundColor: randomColor, display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'whitesmoke', fontWeight: 'bold' }}>{chat.creator.name?.substring(0, 2).toUpperCase()}</Box>
            </Grid>
            {/* info container */}
            <Grid style={{ minWidth: '85%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                ...
                <Grid style={{ margin: '0px 5px', display: 'flex', color: !selected && 'rgb(112,117,121)', lineHeight: '1.5rem', overflow: 'hidden' }}>
